...
#define MAX 1001    
#define WALL_LIMIT 1  // Max number of walls we can remove

int n, m;
int arr[MAX][MAX];
int queue[MAX*MAX*2][3];  // Queue now holds x, y, and the number of walls removed
int dist[MAX][MAX][WALL_LIMIT + 1];  // 3D dist array
int dx[4] = {-1, 1, 0, 0};
int dy[4] = {0, 0, -1, 1};
int front, rear;
int wallCnt;
...
int bfs() {
    front = 0, rear = 0;
    push(0, 0, 0);  // Start with no walls removed
    dist[0][0][0] = 1;
    ...
    for(int i = 0; i < n; i++) {
        for(int j = 0; j < m; j++) {
            ...
            for(int k = 0; k <= WALL_LIMIT; k++) {
                dist[i][j][k] = -1;  // Initialize dist for all wall removal counts
            }
        }
    }
